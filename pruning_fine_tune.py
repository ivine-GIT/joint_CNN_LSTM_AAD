...
for epoch in range(max_epochs):
    ...
    for batch_E, batch_A, batch_y in trainLoader:
        ...
        if 0 == epoch and 1 == iter:
            pruning_started = True
            prune_utils.prune_model(model, prune_percent)
            local_mask = prune_utils.get_mask(model)
            prune_utils.rem_pruned_weights(model)
            prune_utils.print_sparse_percent(model)

        optimizer.step()    # Does the update
        if pruning_started:
            prune_utils.update_weights(model, local_mask)
